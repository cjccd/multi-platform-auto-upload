...
description: 当 agent 需要通过已安装的 `mpau` CLI 完成 Bilibili 登录、账号校验或视频上传时使用这个 skill。优先使用 `mpau bilibili ...`，程序会自动准备 `biliup`，不要求用户手动安装。
---
...
| 功能 | 命令入口 | 说明 |
| --- | --- | --- |
| 登录 | `mpau bilibili login --account <name>` | 需要用户自己在本地真实终端里执行，用于生成或刷新登录信息 |
| 校验 | `mpau bilibili check --account <name>` | 检查指定账号当前是否有效 |
| 视频上传 | `mpau bilibili upload-video ...` | 上传一条 Bilibili 视频 |

## 默认工作流

1. 先确认 `references/runtime-requirements.md`
2. 再确认 `references/cli-contract.md`
3. 执行匹配的 `mpau bilibili ...` 命令
...
- 优先确认当前环境能运行 `mpau`
- 如果 `mpau` 不在 PATH 中，可以用仓库里的 `mpau_cli.py`
- 不要要求用户手动下载 `biliup`
- 第一次运行 Bilibili 命令时，程序可能会自动联网准备 `biliup`
- 对 agent 来说，不要在非交互环境里硬跑 `mpau bilibili login`
- 正确做法是让用户自己在本地终端执行 `mpau bilibili login --account <name>`
