...
- 已安装 `multi-platform-auto-upload`
- 可以调用 `mpau`，或者至少可以执行 `python mpau_cli.py`
...
```powershell
.\.venv\Scripts\python.exe mpau_cli.py bilibili --help
```

bash / zsh：

```bash
python mpau_cli.py bilibili --help
```
...
- `mpau bilibili login --account <name>` 需要用户自己在本地真实终端里执行
